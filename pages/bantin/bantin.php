    ...
    <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-3   bg-gradient-dark" id="sidenav-main">
      ...
      <div class="collapse navbar-collapse  w-auto " id="sidenav-collapse-main">
        <ul class="navbar-nav">
          ...
          <li class="nav-item">
            <a class="nav-link text-white  active bg-gradient-primary" href="bantin.php">
              <div class="text-white text-center me-2 d-flex align-items-center justify-content-center">
            ...
            <div class="ms-md-auto pe-md-3 d-flex align-items-center">
              <form action="" method="GET">
                <div class="input-group input-group-outline">
                  ...
                  <input style="background-color: #E91E63;color: white; margin: 0px 0px 0px -1px; padding: 10px 12px; border-radius: 10%; border-width: 0ch;" type="submit" name="submit" value="Tìm" />
                </div>
              </form>
              <?php 
              include '../../connectsql.php';
              if (isset( $_GET['submit']) && $_GET["search"] != ''){
                $search = $_GET['search'];
                $query = "SELECT * FROM bantin WHERE BT_CHUDE like '%$search%'";
                $sql = mysqli_query($con, $query);
                $num = mysqli_num_rows($sql);
                if ($num > 0) {
                  echo $num." kết quả trả về với <b>".$search."</b>";
                  // echo '<table border="1" cellspacing="0" cellpadding="10">';
                  // foreach( $sql as $row ) {
                  //     echo '<tr>';
                  //         echo "<td>{$row['BT_CHUDE']}</td>";
                  //         echo "<td>{$row['BT_NOIDUNG']}</td>";
                  //     echo '</tr>';
                  // }
                  // echo '</table>';
              } 
            ...
            <div class="card my-4">
              <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                  <h6 class="text-white text-capitalize ps-3" style="text-align: center;">BẢN TIN</h6>
                </div>
              </div>
              <?php
              //  message
              ...
              <div style="align-self:center;">
                <td><a href='thembt.php'><button class='btn btn-dark'>Thêm</button></a></td>
              </div>
              ...
              <div class="card-body px-0 pb-2">
                <div class="table-responsive p-0">
                  <table class="table align-items-center mb-0" id="table-data">
                    <thead>
                      <tr>
                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Sửa</th>
                        ...
                        <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">Chủ đề</th>
                    ...
                    <tbody class="danhsach">
                      <?php
                      $query = "SELECT * FROM bantin ";
                      $query_run = mysqli_query($con, $query);
                      if (mysqli_num_rows($query_run) > 0) {
                        foreach ($query_run as $row) {
                      ?>
                          <tr>
                            <td><a href='suabt.php?id_bt=<?php echo $row['BT_ID'] ?>'><label class='badge badge-sm bg-gradient-success'>Sửa</label></a></td>
                            <td><a onclick="return Del('<?php echo $row['BT_CHUDE']; ?>')" href='xoabt.php?id_bt=<?php echo $row['BT_ID'] ?>'><label class='badge badge-sm bg-gradient-secondary'>Xóa</label></a></td>
                            <td><?php echo $row['BT_ID']; ?></td>
                            <td>
                              <img style="width:50px; border-radius: 50%; height:50px;" src="../../image/bantin/<?php echo $row['BT_ANH']; ?>">
                            </td>
                            <td><?php echo $row['BT_CHUDE']; ?></td>
                            <td><?php echo $row['BT_NOIDUNG']; ?></td>
